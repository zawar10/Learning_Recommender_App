...
# Optional column: default roadmap by category when missing or blank
ROADMAP_FALLBACK_BY_CATEGORY = {
    'Programming': 'https://roadmap.sh/python',
    'Computer Science': 'https://roadmap.sh/computer-science',
    'Database': 'https://roadmap.sh/sql',
    'Machine Learning': 'https://roadmap.sh/ai-data-scientist',
    'Deep Learning': 'https://roadmap.sh/ai-data-scientist',
    'NLP': 'https://roadmap.sh/ai-data-scientist',
    'Computer Vision': 'https://roadmap.sh/ai-data-scientist',
    'Data Viz': 'https://roadmap.sh/data-analyst',
    'Mathematics': 'https://roadmap.sh/data-analyst',
    'Web Dev': 'https://roadmap.sh/full-stack',
    'DevOps': 'https://roadmap.sh/devops',
    'Tools': 'https://roadmap.sh/backend',
    'Cloud': 'https://roadmap.sh/devops',
    'AI Ethics': 'https://roadmap.sh/ai-data-scientist',
    'Big Data': 'https://roadmap.sh/data-engineer',
    'MLOps': 'https://roadmap.sh/devops',
}

if 'roadmap_url' not in courses.columns:
    courses['roadmap_url'] = courses['category'].map(ROADMAP_FALLBACK_BY_CATEGORY).fillna(
...
def _roadmap_url_for_row(row):
    raw = _safe_str(row.get('roadmap_url', ''), '')
    if raw:
        return raw
    cat = row.get('category', '')
    return ROADMAP_FALLBACK_BY_CATEGORY.get(
        cat if isinstance(cat, str) else str(cat),
        'https://roadmap.sh/computer-science',
    )


def _course_api_fields(info):
    """Shared description + roadmap_url for API payloads."""
    return {
        'description': _safe_str(info.get('description', ''), ''),
        'roadmap_url': _roadmap_url_for_row(info),
    }
...
def get_cf_recommendations(user_id, n=10):
    ...
    for course_id, pred in scored:
        ...
        row.update(_course_api_fields(info))
        result.append(row)
    return result
...
def get_cb_recommendations(course_ids, n=10):
    ...
    merge_cols = ['course_id', 'course_name', 'category', 'difficulty', 'description']
    if 'roadmap_url' in courses.columns:
        merge_cols.append('roadmap_url')
    result = sim_df.merge(courses[merge_cols], on='course_id')
    records = result.to_dict(orient='records')
    for rec in records:
        if 'roadmap_url' not in rec or not _safe_str(rec.get('roadmap_url', ''), ''):
            info = courses[courses['course_id'] == rec['course_id']].iloc[0]
            rec['roadmap_url'] = _roadmap_url_for_row(info)
    return records
...
def generate_learning_path(user_id, n_courses=8):
    ...
    for i, row in path.iterrows():
        ...
        info = courses[courses['course_id'] == int(row['course_id'])].iloc[0]
        entry = {
            'step'        : i + 1,
        ...
        entry.update(_course_api_fields(info))
...
def get_all_courses():
    ...
    for _, row in courses.iterrows():
        desc_str = _safe_str(row.get('description', ''), '')
        c_dict = {
            'course_id': int(row['course_id']),
            'course_name': row['course_name'],
            'category': row['category'],
            ...
            'roadmap_url': _roadmap_url_for_row(row),
        }
...
def generate_custom_recommendations(completed_courses, skill_level, career_goal, n_recommendations=10):
    ...
    for course_id, score in scored:
        info = courses[courses['course_id'] == course_id].iloc[0]
        fields = _course_api_fields(info)
        result.append({
            'course_id'   : int(course_id),
            'course_name' : info['course_name'],
            'category'    : info['category'],
            ...
            'roadmap_url': fields['roadmap_url'],
...
def generate_custom_learning_path(recommendations, hours_wk=10):
    ...
    for _, row in df.iterrows():
        ...
        r_url = row.get('roadmap_url', np.nan)
        if pd.isna(r_url) or not str(r_url).strip():
            info = courses[courses['course_id'] == int(row['course_id'])].iloc[0]
            r_url = _roadmap_url_for_row(info)
        else:
            r_url = str(r_url).strip()
        path_list.append({
